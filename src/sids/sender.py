...
def send_negatives(count=10):
    ...
    for signature in RULES:
        ...
        if not (
            signature.srcPort.startswith("!") or signature.srcPort in ["any", "none"]
        ):
        ...
        if not (
            signature.dstPort.startswith("!") or signature.dstPort in ["any", "none"]
        ):
...
def send_positives():
    ...
    for signature in RULES:
        if (
            signature.srcIP == ip or (signature.dir == "<>" and signature.dstIP == ip)
        ) and not (
            signature.dstIP.startswith("!")
            or signature.srcPort.startswith("!")
            or signature.dstPort.startswith("!")
        ):
