...
class Rule:
    ...
    def __init__(self, str):
        ...
        if len(strs) >= 7:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid rule : incorrect source ips : '" + strs[2] + "'."
                )
            try:
                self.srcPorts = Ports(strs[3])
            except:
                raise ValueError(
                    "Invalid rule : incorrect source ports : '" + strs[3] + "'."
                )
            ...
            try:
                ...
                raise ValueError(
                    "Invalid rule : incorrect destination ips : '" + strs[5] + "'."
                )
            ...
            try:
                ...
                raise ValueError(
                    "Invalid rule : incorrect destination ports : '" + strs[6] + "'."
                )
                ...
                for opt in opts:
                    ...
                    if len(kv) >= 2:
                        ...
                        if option == "msg":
                            ...
                            raise ValueError(
                                "Invalid rule : incorrect option : '" + option + "'."
                            )
            # Done
        else:
            raise ValueError(
    ...
    def checkIps(self, pkt):
        ...
        if IP not in pkt:
            ...
            srcIp = pkt[IP].src
            dstIp = pkt[IP].dst
            ipSrc = ip_address(unicode_(srcIp))
            ipDst = ip_address(unicode_(dstIp))
